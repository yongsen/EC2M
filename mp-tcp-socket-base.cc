...
MpTcpSocketBase::reduceCWND (uint8_t sFlowIdx)
{
    ...
    switch ( m_algoCC )
    {
        case Uncoupled_TCPs:
            ...
            //NS_LOG_ERROR (Simulator::Now().GetSeconds() <<" MpTcpSocketBase -> "<< " m_localToken "<< m_localToken<<" Subflow "<< (int)sFlowIdx <<": RTT "<< sFlow->rtt->GetCurrentEstimate().GetSeconds() <<" reducing cwnd from " << cwnd << " to "<<sFlow->cwnd <<" Throughput "<< (sFlow->cwnd * sFlow->MSS * 8) / sFlow->rtt->GetCurrentEstimate().GetSeconds()<< " GlobalThroughput "<<gThroughput<< " Efficacity " <<  getConnectionEfficiency() << " delay "<<lDelay << " Uncoupled_TCPs" );
            // Logging performance metrics into output files
            if(sFlowIdx == 0)
            {
            std::ofstream outfile ("uncoupled0", std::ios::app);

            //Time, RTT, last cwnd, current cwnd, Throughput, GlobalThroughput, Efficacity, delay, alpha);
            outfile << Simulator::Now().GetSeconds() <<" "<< sFlow->rtt->GetCurrentEstimate().GetSeconds() <<" " << cwnd << " "<<sFlow->cwnd <<" "<< (sFlow->cwnd * sFlow->MSS * 8) / sFlow->rtt->GetCurrentEstimate().GetSeconds() << " "<<gThroughput<< " " <<  getConnectionEfficiency() << " "<<lDelay <<" "<< alpha << "\n";
            outfile.close();
            }
            else if(sFlowIdx == 1)
            {
            std::ofstream outfile ("uncoupled1", std::ios::app);
        ...
        case Linked_Increases:
            ...
            if(sFlowIdx == 0)
            {
            std::ofstream outfile ("linked0", std::ios::app);
            ...
            {
            std::ofstream outfile ("linked1", std::ios::app);
        ...
        case RTT_Compensator:
            ...
            if(sFlowIdx == 0)
            {
            std::ofstream outfile ("compensator0", std::ios::app);
            ...
            {
            std::ofstream outfile ("compensator1", std::ios::app);
        ...
        case Fully_Coupled:
            ...
            //Time, m_localToken, Subflow, RTT, last cwnd, current cwnd, Throughput, GlobalThroughput, Efficacity, delay, alpha);
            //NS_LOG_ERROR (Simulator::Now().GetSeconds() <<" "<< " "<< m_localToken<<" "<< (int)sFlowIdx <<" "<< sFlow->rtt->GetCurrentEstimate().GetSeconds() <<" " << cwnd << " "<<sFlow->cwnd <<" "<< (sFlow->cwnd * sFlow->MSS * 8) / sFlow->rtt->GetCurrentEstimate().GetSeconds() << " "<<gThroughput<< " " <<  getConnectionEfficiency() << " "<<lDelay <<" "<< alpha);
            // Logging performance metrics into output files
            if(sFlowIdx == 0)
            {
            std::ofstream outfile ("fully0", std::ios::app);
            ...
            {
            std::ofstream outfile ("fully1", std::ios::app);
